$("#exportHtml").click(function() {
  if ($('textarea[name=usrcsv]').val() == "") {
	...
	var newCode = '<pre id="' + preid + '" style="display:none;">\n' + usrcsv + '\n' + '</pre>\n' + '<div id="' + divid + '"></div>\n' + '<script>\n' +'$(function(){var t=$("#' + preid + '").html().replace(/[ \t]+/g,",");Highcharts.chart("' + divid + '",{chart:{type:"scatter",zoomType:"xy",style:{fontFamily:"Arial"}},xAxis:{title:{text:"' + xtitle + '"},plotLines:[{color:"' + lineColor + '",value:' + X1 + ',width:1,dashStyle:"dashdot",zIndex:5},{color:"' + lineColor + '",value:' + X2 + ',width:1,dashStyle:"dashdot",zIndex:5}]},yAxis:{title:{text:"' + ytitle + '"},labels:{format:"{value}"},plotLines:[{color:"' + lineColor + '",value:' + Y1 + ',width:1,dashStyle:"dashdot",zIndex:5},{color:"' + lineColor + '",value:' + Y2 + ',width:1,dashStyle:"dashdot",zIndex:5}]},data:{csv:t,seriesMapping:[{x:1,y:2,name:0}]},plotOptions:{scatter:{dataLabels:{enabled:' + showLabels + ',format:"{point.name}"},marker:{fillColor:"' + pcolor + '",radius:3,symbol:"circle"},cursor:"pointer",point:{events:{click:function(){var l="' + preurl + '"+this.options.name;window.open(l)}}},turboThreshold:0}},title:{text:"' + mtitle + '"},legend:{enabled:!1},credits:{enabled:!1},tooltip:{formatter:function(){return this.point.name+"<br/>' + xtitle + ': "+Highcharts.numberFormat(this.x,2)+"<br>' + ytitle + ': "+Highcharts.numberFormat(this.y,2)}}})});' + '</script>';
	alert(newCode);
  };
});
